...
enum X1EpaperRefreshState { acknowledged, unknown }
...
class X1NfcEpaperProtocol {
  ...
  static X1EpaperRefreshState parseRefreshState(Uint8List response) {
    if (!hasExactSuccessStatus(response) || response.length < 3) {
      return X1EpaperRefreshState.unknown;
    }
    return switch (response[response.length - 3]) {
      0x00 || 0x01 => X1EpaperRefreshState.acknowledged,
      _ => X1EpaperRefreshState.unknown,
    };
  }
