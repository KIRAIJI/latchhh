...
class _EpaperWriteModalState extends State<EpaperWriteModal>
  ...
  _EpaperWritePhase _resolvePhase() {
    ...
    if (lower.contains('retry refresh')) {
      return _EpaperWritePhase.retry;
    }
                ...
                children: [
                  ...
                  Text(
                    phase == _EpaperWritePhase.waiting
                        ...
                        : phase == _EpaperWritePhase.retry
                        ? 'Lift the phone away, move it slightly, tap Retry '
                              'Refresh, then place it back on the NFC area.'
                  ...
                  else if (phase == _EpaperWritePhase.retry)
                    LatchButton(
                      label: 'Retry Refresh',
                      variant: LatchButtonVariant.primary,
                      fullWidth: true,
                      enabled: !_isBusy,
                      onPressed: _beginWrite,
                    )
                  else
                    LatchButton(
                      label: 'Done',
...
enum _EpaperWritePhase { waiting, success, failure, retry }
...
class _NfcScanIndicator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final iconColor = switch (phase) {
      ...
      _EpaperWritePhase.retry => AppColors.primary,
