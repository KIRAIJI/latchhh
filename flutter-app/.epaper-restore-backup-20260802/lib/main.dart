...
import 'latch_ui/features/epaper/data/x1_epaper_adaptive_frame.dart';
...
class _EpaperRefreshPendingException implements Exception {
  const _EpaperRefreshPendingException();

  static const String userMessage =
      'Image transferred. Lift your phone away, reposition it, then tap '
      'Retry Refresh.';

  @override
  String toString() => 'E-paper image transferred; display refresh pending.';
}
...
class _UploaderPageState extends State<UploaderPage> {
  ...
  bool _refreshOnlyPending = false;
  bool _refreshRetryUseAlternateMode = true;
  ...
  Future<void> _ingestSelectedImageBytes(Uint8List bytes) async {
    setState(() {
      ...
      _refreshOnlyPending = false;
      _refreshRetryUseAlternateMode = true;
  ...
  void _rebuildImagePipeline() {
    ...
    img.Image working = _resizeToCover(
      decoded,
      swapWh ? processH : processW,
      swapWh ? processW : processH,
      interpolation: strictApkClone
          ? img.Interpolation.linear
          : (_useApkColorMapping && !_fitToTagCapacity && _apkPixelizedResize)
    ...
    final _PreparedFrame prepared;
    if (strictApkClone) {
      final X1AdaptiveFrame adaptive = buildX1AdaptiveFrame(
        source: working,
        scanMode: _apkScanMode == _ApkScanMode.vertical
            ? X1EpaperScanMode.vertical
            : X1EpaperScanMode.horizontal,
      );
      prepared = _PreparedFrame(
        previewPng: Uint8List.fromList(img.encodePng(adaptive.preview)),
        packed: adaptive.packed,
      );
      _appendLog(
        'Image profile: ${adaptive.profile}; '
        '${adaptive.fragmentCount} fragments, '
        '${adaptive.compressedBytes} compressed bytes, '
        '${(adaptive.inkCoverage * 100).toStringAsFixed(1)}% ink.',
      );
    } else if (_useApkColorMapping && !_fitToTagCapacity) {
      prepared = _prepareApkColorFrame(
        working,
        ...
        photoPreserve: _photoPreserveQuantization,
        codeWhite: (int.tryParse(_codeWhiteController.text.trim()) ?? 0).clamp(
          0,
          3,
        ),
        codeBlack: (int.tryParse(_codeBlackController.text.trim()) ?? 3).clamp(
          0,
          3,
        ),
        codeRed: (int.tryParse(_codeRedController.text.trim()) ?? 2).clamp(
          0,
          3,
        ),
        codeYellow: (int.tryParse(_codeYellowController.text.trim()) ?? 1)
            .clamp(0, 3),
      );
    } else {
      prepared = _prepareMonochromeFrame(
        working,
        threshold: _threshold,
        invert: _invert,
        dither: _dither,
  ...
  Future<bool> _apkExactRefreshPoll(
    ...
    for (int i = 0; i < maxPolls; i++) {
      final Uint8List poll = await _sendApdu(
        isoDep,
        X1NfcEpaperProtocol.refreshPoll,
        label: 'apk exact poll ${i + 1}',
        failOnStatus: false,
      );
      if (poll.length < 2) {
        throw StateError('Invalid F0DE refresh response.');
      }
      final String pollHex = _responseHex(poll);
      final X1EpaperRefreshState refreshState =
          X1NfcEpaperProtocol.parseRefreshState(poll);
      if (refreshState == X1EpaperRefreshState.acknowledged) {
        _appendLog(
          'F0DE acknowledged the refresh after ${i + 1} polls: $pollHex.',
      ...
      if (pollHex == '68C6') {
        if (!firstBranchUsed) {
          ...
          final bool branchRefreshed = await _apkExactRefreshTrigger(
            isoDep,
      ...
      _appendLog(
        'F0DE returned an unsupported response: $pollHex.',
        level: _LogLevel.error,
      );
      return false;
    }
    _appendLog('F0DE exhausted max polls ($maxPolls).', level: _LogLevel.error);
    return false;
  }
  ...
  Future<bool> _apkExactRefreshTrigger(
    ...
    while (true) {
      ...
      if (triggerHex == '019000') {
        _appendLog(
          'F0D4 directly acknowledged the refresh: $triggerHex.',
          level: _LogLevel.success,
        );
        return true;
      }
      if (triggerHex == '009000' || triggerHex == '9000') {
        _appendLog(
          'F0D4 returned $triggerHex; checking the vendor refresh result...',
          level: _LogLevel.info,
        );
      ...
      if (fallbackHex == '9000') {
        _appendLog(
          'F0D4 fallback acknowledged the refresh: $fallbackHex.',
          level: _LogLevel.success,
        );
        return true;
      }
  ...
  Future<bool> _runApkRefreshWithRetries(
    ...
    for (int attempt = 1; attempt <= safeAttempts; attempt++) {
      if (mounted) {
        setState(() {
          _status =
              'Refreshing E-Paper… Attempt $attempt/$safeAttempts. '
      ...
      if (attempt < safeAttempts && gapMs > 0) {
        if (mounted) {
          setState(() {
            _status =
                'Recharging the NFC tag before refresh attempt '
                '${attempt + 1}/$safeAttempts… Keep your phone in place.';
          });
        }
        _appendLog(
          'Recharging the NFC tag for ${gapMs}ms before retry...',
          level: _LogLevel.info,
        );
  ...
  Future<void> _writeTag() async {
    ...
    setState(() {
      _busy = true;
      _status = _refreshOnlyPending
          ? 'Hold your phone against the repositioned NFC area. Only the '
                'pending display refresh will be retried.'
        ...
        await NfcManager.instance.startSession(
          ...
          onDiscovered: (NfcTag tag) async {
            ...
            try {
              ...
              _appendLog(
                'Tag discovered. Techs: ${androidTag?.techList.join(', ') ?? 'unknown'}',
              );
              if (_mode == NfcWriteMode.ndef) {
                await _writeAsNdef(tag, payload);
              } else {
                await _writeAsIsoDep(tag, payload, isResume: false);
              }
            ...
            } on _EpaperRefreshPendingException catch (e) {
              if (sessionId != _writeSessionId) {
                return;
              }
              completed = true;
              ...
              setState(() {
                _busy = false;
                _status = _EpaperRefreshPendingException.userMessage;
              });
              _appendLog('$e', level: _LogLevel.warning);
            } catch (e) {
              if (sessionId != _writeSessionId) {
                return;
              }
  ...
  Future<void> _writeAsIsoDepApkExact(
    ...
    _appendLog(
      'APK exact path active. maxTx=$maxTx; payload=${payload.length} bytes; resumeBlockOverride=$requestedStartBlock.',
    );
    ...
    if (_refreshOnlyPending) {
      final bool useAlternateMode = _refreshRetryUseAlternateMode;
      _appendLog(
        'Retrying the pending display refresh without uploading image data '
        'using mode ${useAlternateMode ? '0x85' : '0x05'}.',
        level: _LogLevel.info,
      );
      if (mounted) {
        setState(() {
          _status =
              'Charging the repositioned NFC tag before retrying the '
              'display refresh…';
        });
      }
      await Future<void>.delayed(const Duration(milliseconds: 1500));

      final bool pendingRefreshCompleted;
      try {
        pendingRefreshCompleted = await _apkExactRefreshTrigger(
          isoDep,
          refreshIndex: 0,
          firstBranchUsed: useAlternateMode,
          variantMode85Used: useAlternateMode,
        );
      } catch (error) {
        _refreshRetryUseAlternateMode = !useAlternateMode;
        if (_isLikelyTagLostError(error)) {
          _appendLog(
            'The NFC field dropped during the pending refresh. Keeping the '
            'image ready for another repositioned refresh-only attempt.',
            level: _LogLevel.warning,
          );
        }
        throw const _EpaperRefreshPendingException();
      }
      if (!pendingRefreshCompleted) {
        _refreshRetryUseAlternateMode = !useAlternateMode;
        throw const _EpaperRefreshPendingException();
      }
      ...
      _refreshRetryUseAlternateMode = true;
      _appendLog(
        'Pending display refresh completed without re-uploading the image.',
        level: _LogLevel.success,
      );
      return;
    }
    ...
    if (deviceInfo.supportsCompression) {
      ...
      _appendLog(
        ...
        'data=$compressedBytes/${effectivePayload.length} bytes.',
      );
      ...
      for (int i = startBlock; i < blocks; i++) {
        ...
        try {
          ...
          if (_isLikelyTagLostError(e)) {
            _f0StartBlockController.text = '0';
            _appendLog(
              'APK exact transfer was interrupted at block $i.',
              level: _LogLevel.warning,
            );
            throw StateError('F0D2_TAG_LOST_AT_BLOCK:$i');
          }
    ...
    _refreshOnlyPending = true;
    _refreshRetryUseAlternateMode = true;
    if (mounted) {
      setState(() {
        _status =
            'Charging the NFC tag for the E-Paper refresh… '
            'Keep your phone in place.';
      });
    }

    // The X1 tag is powered by the phone's NFC field. Complex four-color
    // frames can leave too little stored energy immediately after upload,
    // causing the refresh command to be refused with 68CA. A quiet interval
    // lets the tag recharge without retransmitting the already accepted image.
    await Future<void>.delayed(const Duration(milliseconds: 2500));

    final bool refreshed;
    try {
      refreshed = await _runApkRefreshWithRetries(
        isoDep,
        attempts: 1,
        gapMs: 0,
      );
      ...
      if (_isLikelyTagLostError(e)) {
        _appendLog(
          'The NFC connection was interrupted during the display refresh.',
          level: _LogLevel.warning,
        );
      }
      throw const _EpaperRefreshPendingException();
...
_PreparedFrame _prepareApkColorFrame(
  ...
  int nearest4Palette(int r, int g, int b) {
    if (!photoPreserve) {
      // Strict APK BMPConverterUtil.findNearestColor behavior.
      // Class indices used by this app: 0=white, 1=black, 2=red, 3=yellow.
