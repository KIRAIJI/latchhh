...
class NfcManagerPlugin: FlutterPlugin, ActivityAware, HostApiPigeon, BroadcastReceiver(),
  PluginRegistry.NewIntentListener, NfcAdapter.ReaderCallback {
  private lateinit var flutterApi: FlutterApiPigeon
  private lateinit var activity: Activity
  private var activityBinding: ActivityPluginBinding? = null
  private var adapter: NfcAdapter? = null
  private var cachedTags: MutableMap<String, Tag> = ConcurrentHashMap()
  ...
  override fun nfcAdapterEnableReaderMode(flags: List<ReaderFlagPigeon>) {
    ...
    nfcAdapter.enableReaderMode(activity, this, 0x11f, readerOptions)
  ...
  override fun nfcAdapterDisableReaderMode() {
    if (foregroundDispatchEnabled) {
      try {
        getAdapter().disableForegroundDispatch(activity)
  ...
  private inline fun <reified T: TagTechnology> forceConnect(handle: String, getMethod: (Tag) -> T?): T {
    ...
    if (
      ...
      activeTech != null &&
      activeTech::class.java.name == tech::class.java.name &&
      activeTech.isConnected
    ) {
      @Suppress("UNCHECKED_CAST")
      return activeTech as T
    }
