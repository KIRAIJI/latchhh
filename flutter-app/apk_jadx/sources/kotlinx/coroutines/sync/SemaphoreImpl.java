...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM1949constructorimpl;
        boolean z;
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM1949constructorimpl = SegmentOrClosed.m1949constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM1949constructorimpl = SegmentOrClosed.m1949constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (SegmentOrClosed.m1954isClosedimpl(objM1949constructorimpl)) {
                break;
            }
            Segment segmentM1952getSegmentimpl = SegmentOrClosed.m1952getSegmentimpl(objM1949constructorimpl);
            while (true) {
                Segment segment = (Segment) this.tail;
                if (segment.getId() >= segmentM1952getSegmentimpl.getId()) {
                    break;
                }
                if (!segmentM1952getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    break;
                }
                if (tail$FU.compareAndSet(this, segment, segmentM1952getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (segmentM1952getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    segmentM1952getSegmentimpl.remove();
                }
            }
            z = true;
        } while (!z);
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m1952getSegmentimpl(objM1949constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        do {
            ...
            while (true) {
                ...
                if (head$FU.compareAndSet(this, segment, segmentM1952getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (segmentM1952getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
