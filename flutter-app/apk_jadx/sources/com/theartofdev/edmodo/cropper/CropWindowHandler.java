...
final class CropWindowHandler {
    ...
    public CropWindowMoveHandler getMoveHandler(float f, float f2, float f3, CropImageView.CropShape cropShape) {
        ...
        if (cropShape == CropImageView.CropShape.OVAL) {
            rectanglePressedMoveType = getOvalPressedMoveType(f, f2);
        } else {
            rectanglePressedMoveType = getRectanglePressedMoveType(f, f2, f3);
        }
    ...
    private CropWindowMoveHandler.Type getOvalPressedMoveType(float f, float f2) {
        ...
        float f4 = this.mEdges.left + (fWidth * 5.0f);
        float fHeight = this.mEdges.height() / 6.0f;
        float f5 = this.mEdges.top + fHeight;
        float f6 = this.mEdges.top + (fHeight * 5.0f);
        if (f < f3) {
            if (f2 < f5) {
                return CropWindowMoveHandler.Type.TOP_LEFT;
            }
