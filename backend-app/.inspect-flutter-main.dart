...
class _UploaderPageState extends State<UploaderPage> {
  ...
  void _rebuildImagePipeline() {
    ...
    img.Image working = _resizeToCover(
      ...
      interpolation:
          (_useApkColorMapping &&
              !_fitToTagCapacity &&
              (strictApkClone || _apkPixelizedResize))
    ...
    for (int i = 0; i < maxPolls; i++) {
      ...
      try {
        ...
        if (_isLikelyTagLostError(e)) {
          _appendLog(
            'F0DE: tag lost before an acknowledged refresh result.',
            level: _LogLevel.warning,
          );
        }
        rethrow;
      ...
      if (pollHex == '009000' || pollHex == '019000') {
        _appendLog(
          'F0DE poll success: $pollHex after $i attempts',
  ...
  Future<void> _writeTag() async {
    ...
    const int maxRediscoveries = 100;
          ...
          onDiscovered: (NfcTag tag) async {
            ...
            try {
              ...
              if (rediscoveryAttempts > 1 && mounted) {
                ...
                _appendLog(
                  'X1 recovery: starting a fresh transaction from block 0.',
                  level: _LogLevel.info,
                );
              }
              ...
              if (_isRefreshTagLostError(e) &&
                ...
                _f0StartBlockController.text = '0';
                _appendLog(
                  'Tag lost during refresh stage. Restarting reader mode to '
                  'repeat the complete X1 transaction from block 0 (attempt '
                  '$rediscoveryAttempts/$maxRediscoveries).',
                  level: _LogLevel.warning,
                );
                if (!mounted) {
  ...
  Future<void> _writeAsIsoDepApkExact(
    ...
    _appendLog(
      'APK exact path active. maxTx=$maxTx; payload=${payload.length} bytes; resumeBlockOverride=$requestedStartBlock; refreshOnly=$refreshOnly.',
    );
    ...
    if (!refreshOnly) {
      ...
      _appendLog(
        'APK exact refresh-only mode active. Skipping F0D2 upload and retrying refresh trigger.',
        level: _LogLevel.warning,
      );
    }
    ...
    try {
      refreshed = await _apkExactRefreshTrigger(
        isoDep,
        refreshIndex: 0,
        firstBranchUsed: false,
        variantMode85Used: false,
      ...
      if (_isLikelyTagLostError(e)) {
        _appendLog(
          'Tag lost during APK exact refresh trigger. Deferring refresh to rediscovery.',
          level: _LogLevel.warning,
        );
